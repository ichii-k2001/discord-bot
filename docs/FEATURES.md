...
### 概要
予定の追加、表示、管理機能を提供します。将来的にGoogleカレンダーとの連携も予定。
...
### 概要
タスクの追加、管理、期限確認機能を提供します。優先度設定や期限切れ警告も含まれます。
...
### 概要
各ユーザーが個別にデータの表示方法を制御できる機能です。

### `/privacy_mode <モード> [機能]`
プライバシーモードを設定します。

**パラメータ:**
- `mode`: shared（共有）または private（プライベート）
- `feature`: calendar（カレンダー）、tasks（タスク）、all（すべて）

**使用例:**
```bash
# タスクをプライベートモードに
/privacy_mode private tasks

# すべての機能を共有モードに
/privacy_mode shared all
```
...
**動作モード:**
- **共有モード**: サーバー内の全ユーザーのデータを表示
- **プライベートモード**: 自分が作成したデータのみ表示

**注意事項:**
- Google連携使用時は、実際のカレンダー・スプレッドシートでは全データが見えます
- Discord上での表示フィルタリングのみ行われます
...
- ユーザー設定: `data/user_settings.json` (自動生成)
