...
sequenceDiagram
    ...
    alt 詳細表示モード
        ...
        D-->>U: 詳細翻訳結果表示
        TC->>D: followup.send(コピー用テキスト)
        D-->>U: 📋 コピー用テキスト表示
    else シンプル表示モード（デフォルト）
        TC->>TC: 使用統計確認
        alt 使用制限10回以下
            TC->>D: followup.send(text + 警告)
            D-->>U: 翻訳結果 + 使用量警告
        else 使用制限十分
...
sequenceDiagram
    ...
    TC->>TS: translate_text(text, language_input, user_id)
    TS->>TS: _normalize_language_code(language_input)
